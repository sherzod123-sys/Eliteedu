...
class OrderTests(APITestCase):
    ...
    def test_create_order(self):
        ...
        data = {'course_id': self.course.id, 'payment_method': 'payme'}
    ...
    # ----------------------------------------------------
    # 2. Create Order With Promo Code Test
    # ----------------------------------------------------
    def test_create_order_with_promo(self):
        url = reverse('order-list')
        data = {'course_id': self.course.id, 'promo_code': 'DISCOUNT20', 'payment_method': 'payme'}
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    # ----------------------------------------------------
    # 3. Validate Promo Code Test
    # ----------------------------------------------------
    def test_validate_promo_code(self):
        url = reverse('promocode-validate')
        data = {'code': 'DISCOUNT20', 'course_id': self.course.id}
        response = self.client.post(url, data)
